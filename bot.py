...
def spamuser(xtoken, xsecret, username):
	try:
		headers = {
			"User-Agent": "Twitter-iPhone/9.62 iOS/13.3.3 (Apple;iPhone9,1;;;;;1)",
			"Content-Type": "application/json"
		}
		return requests.post("https://api.twitter.com/1.1/users/report_spam.json?screen_name=" +str(username) + "&perform_block=false", headers=headers,auth=OAuth1('3nVuSoBZnx6U4vzUxf5w','Bcs59EFbbsdF6Sl9Ng71smgStWEGwXXKSjYvPVt7qys',xtoken,xsecret,decoding=None), verify=False).json()
	except Exception as e:
		print("spamuser func", e)
...
#saveFile( str(username) + "_report_spam.json", users )
...
yesOrno1 = input("Are you sure to spam users? (yes or no)\n")
if yesOrno1 == "yes":
	with open(str(username) + "_report_spam.json", "a") as l:
		for user in users:
			if user["followers_count"] < int(limit):
				spamuser(X_Token, X_Secret, str(user["screen_name"]))
				print(user["screen_name"] + " spamed.")
				l.write(user["screen_name"] + "\n")